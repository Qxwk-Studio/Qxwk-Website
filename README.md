...
### 🌓 全站黑夜模式
- **9 个页面全部支持**：默认跟随系统深浅色自动切换，无白屏闪烁
- **手动切换**：导航栏太阳/月亮按钮一键切换，选择存 `localStorage` 记忆
- 深色适配覆盖全部页面元素：卡片、导航栏、按钮、错误提示、答题状态、GitHub 贡献图等
...
- 深色配色：编辑 `:root[data-theme="dark"]` 中的对应变量
- 主题切换逻辑在 `<head>` 内的内联脚本中，localStorage key 为 `theme`
